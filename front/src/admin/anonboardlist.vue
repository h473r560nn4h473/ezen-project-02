  ...
  <main>
    <div class="board-wrap">
      ...
      <div class="theme-wrap">
        <div class="theme" @click="goToNotice()">공지사항</div>
        <div class="bar"></div>
        <div class="theme" @click="goToFree()">자유</div>
        <div class="bar"></div>
        <div class="theme active" @click="goToAnon()">익명</div>
        <div class="bar"></div>
          ...
          <div class="search-wrap">
            <div class="search-input">
              <input refs="searchInput" v-model="keyword" type="search" placeholder="검색"
                @keyup.enter="getAnonList(sortSCase, sortNCase)">
              <div class="erase" v-show="keyword.length > 0" @click="keyword = ''"><i class="fa-solid fa-xmark"></i></div>
            </div>
            <button class="search-btn" type="submit" @click="getAnonList(sortSCase, sortNCase)"><i
        ...
        <table class="table">
          ...
          <tbody>
            <tr v-for="(item, index) in pagePostList" :key="index">
              <td><input type="checkbox" class="check" :value="item.ANO_NO" v-model="checkedPosts"></td>
              <td @click=movetocontent(item.ANO_NO)>{{ item.ANO_NO }}</td>
              <td>{{ cutTheContent(item.ANO_TITLE, 10) }}</td>
              <td>{{ cutTheContent(item.ANO_CONTENT, 40) }}</td>
              <td>{{ formatDateTime(item.ANO_DATE) }}</td>
              <td>{{ item.ANO_COUNT }}</td>
              <td>{{ item.ANO_LIKE }}</td>
            </tr>
          </tbody>
        </table>
...
export default {
  data() {
    return {
      ...
      sortNCase: 0,
      optionsN: '최근 작성 순',
      options: ['최근 작성 순', '오래전 작성 순', '조회수 높은 순', '조회수 낮은 순', '추천수 높은 순', '추천수 낮은 순'],
      sortSCase: 0,
      soptionsN: '글 번호',
      soptions: ['글 번호', '글 제목', '글 내용', '회원번호'],
      keyword: '',
      pagePostList: [],
      checkedPosts: [],
      pageNum: 0,
  ...
  methods: {
    ...
    async getAnonList(sortSCaseNum, sortNCaseNum) {
      let keyword = 'none'
      if (this.keyword != '') {
        keyword = this.keyword;
      }
      try {
        const response = await axios.get(`http://localhost:3000/admin/anonlist/${sortSCaseNum}/${sortNCaseNum}/${keyword}`);
    ...
    sortList(sortNum, event) {
      ...
      if (sortNum == 0) {
        ...
        this.sortNCase = 4
        this.optionsN = '추천수 높은 순'
      } else if (sortNum == 5) {
        this.sortNCase = 5
        this.optionsN = '추천수 낮은 순'
      }
      this.getAnonList(this.sortSCase, this.sortNCase)
    },
    ...
    checkAll() {
      if (this.isAllChecked) {
        ...
        this.checkedPosts = this.pagePostList.map(ano => ano.ANO_NO);
      }
    ...
    async deleteSelectedPosts() {
      ...
      if (confirmResult.value) {
        try {
          const response = await axios.delete(`http://localhost:3000/admin/anonlist/delete`, {
            data: {
          ...
          // 사용자 리스트에서 선택한 회원 제거
          this.postList = this.postList.filter(ano => !this.checkedPosts.includes(ano.ANO_NO));
          this.checkedPosts = [];
          this.$swal({
            icon: 'success',
            title: '삭제되었습니다',
