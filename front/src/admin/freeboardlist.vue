  ...
  <main>
    <div class="board-wrap">
      ...
      <div class="theme-wrap">
        ...
        <div class="bar"></div>
        <div class="theme active" @click="goToBoard()">자유</div>
        <div class="bar"></div>
          ...
          <div class="search-wrap">
            <div class="search-input">
              <input refs="searchInput" v-model="keyword" type="search" placeholder="검색"
                @keyup.enter="getBoardList(sortSCase, sortNCase)">
              <div class="erase" v-show="keyword.length > 0" @click="keyword = ''"><i class="fa-solid fa-xmark"></i></div>
            </div>
            <button class="search-btn" type="submit" @click="getBoardList(sortSCase, sortNCase)"><i
...
export default {
  ...
  methods: {
    ...
    async getBoardList(sortSCaseNum, sortNCaseNum) {
      ...
      try {
        const response = await axios.get(`http://localhost:3000/admin/boardlist/${sortSCaseNum}/${sortNCaseNum}/${keyword}`);
    ...
    checkAll() {
      if (this.isAllChecked) {
        ...
        this.checkedPosts = this.pagePostList.map(bod => bod.BOD_NO);
      }
    ...
    async deleteSelectedPosts() {
      ...
      if (confirmResult.value) {
        try {
          const response = await axios.delete(`http://localhost:3000/admin/boardlist/delete`, {
            data: {
          ...
          // 사용자 리스트에서 선택한 회원 제거
          this.postList = this.postList.filter(bod => !this.checkedPosts.includes(bod.BOD_NO));
          this.checkedPosts = [];
          this.$swal({
            icon: 'success',
            title: '삭제되었습니다',
