  ...
  <main>
    <div class="board-wrap">
      ...
      <div class="theme-wrap">
        ...
        <div class="bar"></div>
        <div class="theme active" @click="goToLost()">분실물</div>
      </div>
          ...
          <div class="search-wrap">
            <div class="search-input">
              <input refs="searchInput" v-model="keyword" type="search" placeholder="검색"
                @keyup.enter="getLostList(sortSCase, sortNCase)">
              <div class="erase" v-show="keyword.length > 0" @click="keyword = ''"><i class="fa-solid fa-xmark"></i></div>
            </div>
            <button class="search-btn" type="submit" @click="getLostList(sortSCase, sortNCase)"><i
...
export default {
  ...
  methods: {
    ...
    async getLostList(sortSCaseNum, sortNCaseNum) {
      let keyword = 'none'
      if (this.keyword != '') {
        keyword = this.keyword;
      }
      try {
        const response = await axios.get(`http://localhost:3000/admin/lostlist/${sortSCaseNum}/${sortNCaseNum}/${keyword}`);
    ...
    sortList(sortNum, event) {
      ...
      this.getLostList(this.sortSCase, this.sortNCase)
    },
    ...
    getLostCategoryText(item) {
      return this.lostCategoryTextList[item] || "Unknown";
    },
    ...
    checkAll() {
      if (this.isAllChecked) {
        ...
        this.checkedPosts = this.pagePostList.map(lost => lost.LOST_NO);
      }
    ...
    async deleteSelectedPosts() {
      ...
      if (confirmResult.value) {
        try {
          const response = await axios.delete(`http://localhost:3000/admin/lostlist/delete`, {
            data: {
          ...
          // 사용자 리스트에서 선택한 회원 제거
          this.postList = this.postList.filter(lost => !this.checkedPosts.includes(lost.LOST_NO));
          this.checkedPosts = [];
          this.$swal({
            icon: 'success',
            title: '삭제되었습니다',
