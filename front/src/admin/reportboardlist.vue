  ...
  <main>
    <div class="board-wrap">
      ...
      <div class="theme-wrap">
        ...
        <div class="bar"></div>
        <div class="theme active" @click="goToReport()">신고</div>
        <div class="bar"></div>
          ...
          <div class="search-wrap">
            <div class="search-input">
              <input refs="searchInput" v-model="keyword" type="search" placeholder="검색"
                @keyup.enter="getReportList(sortCCase, sortSCase, sortNCase)">
              <div class="erase" v-show="keyword.length > 0" @click="keyword = ''"><i class="fa-solid fa-xmark"></i></div>
            </div>
            <button class="search-btn" type="submit" @click="getReportList(sortCCase, sortSCase, sortNCase)"><i
      ...
      <div class="post-wrap">
        ...
        <button class="btn-small btn-blue" @click="readSelectedPosts">선택 확인</button>
        <table class="table">
          <thead>
            <tr>
...
export default {
  data() {
    return {
      ...
      sortNCase: 0,
      optionsN: '최근 작성 순',
      options: ['최근 작성 순', '오래전 작성 순'],
      sortSCase: 0,
      soptionsN: '글 번호',
      soptions: ['글 번호', '글 제목', '글 내용', '범주'],
      sortCCase: 0,
      coptionsN: '전체',
      coptions: ['전체', '확인', '미확인'],
      keyword: '',
  ...
  methods: {
    ...
    async getReportList(sortCCaseNum, sortSCaseNum, sortNCaseNum) {
      let keyword = 'none'
      if (this.keyword != '') {
        keyword = this.keyword;
      }
      try {
        const response = await axios.get(`http://localhost:3000/admin/reportlist/${sortCCaseNum}/${sortSCaseNum}/${sortNCaseNum}/${keyword}`);
    ...
    sortList(sortNum, event) {
      ...
      this.getReportList(this.sortCCase, this.sortSCase, this.sortNCase)
    },
    ...
    checkAll() {
      if (this.isAllChecked) {
        ...
        this.checkedPosts = this.pagePostList.map(rep => rep.REP_NO);
      }
    ...
    async deleteSelectedPosts() {
      ...
      if (confirmResult.value) {
        try {
          const response = await axios.delete(`http://localhost:3000/admin/reportlist/delete`, {
            data: {
          ...
          // 사용자 리스트에서 선택한 회원 제거
          this.postList = this.postList.filter(rep => !this.checkedPosts.includes(rep.REP_NO));
          this.checkedPosts = [];
          this.$swal({
            icon: 'success',
            title: '삭제되었습니다',
    ...
    async readSelectedPosts() {
      ...
      if (confirmResult.value) {
        try {
          const response = await axios.post(`http://localhost:3000/admin/reportlist/read`, {
            data: {
              postNos_Read: this.checkedPosts // 배열 형태로 전달
            }
          });
