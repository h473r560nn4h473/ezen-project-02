<template>
      <div class="search-sort-wrap">
        <div class="search-cond">
          ...
          <div class="search-wrap">
            <div class="search-input">
              <input refs="searchInput" v-model="keyword" type="search" placeholder="검색" @keyup.enter="getAnonList(sortSCase, sortACase)">
              <div class="erase" v-show="keyword.length > 0" @click="keyword = ''"><i class="fa-solid fa-xmark"></i></div>
            </div>
            <button class="search-btn" type="submit" @click="getAnonList(sortSCase, sortACase)"><i class="fa-solid fa-search"></i></button>
          </div>
        </div>
        <div class="sort-wrap" :class="{ active: optionsActive }" >
          ...
          <div class="sort-opt-wrap">
            <div class="sort-opt">
              <div class="option-row" v-for="(option, index) in options" :key="index" :class="{ 'selected': option === sortACase }" @click="handleClick(option, index, $event)">
        ...
        <div class="post-wrap">
          <div class="post grid-box" v-for="item in pageAnonList" :key="item.ANO_NO"
          @click=movetocontent(item.ANO_NO)>
            <!-- <span>{{ item.BOD_NO }}</span> 글번호 -->
            <div class="post-title">{{ item.ANO_TITLE }}</div>
            <div class="post-detail">
              <div class="post-author">익명{{ item.USER_NO }}</div>
              <div class="bar"></div>
              <div class="post-date">{{ formatDateTime(item.ANO_DATE) }}</div>
              <div class="bar"></div>
              <div class="post-view"><i class="fa-regular fa-heart"></i>{{ item.ANO_LIKE }}</div>
              <div class="bar"></div>
              <div class="post-view"><i class="fa-regular fa-eye"></i>{{ item.ANO_COUNT }}</div>
            </div>
          </div>
        </div>
        <!-- 페이징  -->
        ...
        <div class="btn-area">
          ...
          <button v-else class="btn-small" @click="write_anon">작성</button>
        </div> 
</template>
...
export default {
  data() {
    return {
      anonList: [], //현재 게시판과 페이지에 맞는 글 리스트
      sortACase: 0,
      optionsN: '최근 순',
  ...
  methods: {
    ...
    sliceList() {
      ...
      this.pageAnonList = this.anonList.slice(start, start + this.onePageCnt);
    },
    async getAnonList(sortSCaseNum, sortACaseNum) {
      let keyword = 'none'
      if (this.keyword != '') {
        keyword = this.keyword;
      }
      try {
        const response = await axios.get(`http://localhost:3000/anon/anon_list/${sortSCaseNum}/${sortACaseNum}/${keyword}`);
        this.anonList = response.data;
        console.log(this.anonList);
        this.pageCnt = Math.ceil(this.anonList.length / this.onePageCnt)
        this.setPage(1);
      } catch (error) {
        console.error(error);
      }
    ...
    sortList(sortNum, event) {
      ...
      this.getAnonList(this.sortSCase, this.sortACase)
    },
    ...
    login() {
      ...
      movetocontent(ANO_NO) {
      window.location.href = window.location.pathname + '/anonDetail?ANO_NO=' + ANO_NO
    },
