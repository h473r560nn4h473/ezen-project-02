<template>
  <div class="search-sort-wrap">
    <div class="search-cond">
      ...
      <div class="search-wrap">
        <div class="search-input">
          <input refs="searchInput" v-model="keyword" type="search" placeholder="검색" @keyup.enter="getReportList(sortCCase, sortSCase, sortRCase)">
          <div class="erase" v-show="keyword.length > 0" @click="keyword = ''"><i class="fa-solid fa-xmark"></i></div>
        </div>
        <button class="search-btn" type="submit" @click="getReportList(sortCCase, sortSCase, sortRCase)"><i class="fa-solid fa-search"></i></button>
      </div>
    </div> 
    ...
    <div class="sort-wrap" :class="{ active: optionsActive }" >
      ...
      <div class="sort-opt-wrap">
        <div class="sort-opt">
          <div class="option-row" v-for="(option, index) in options" :key="index" :class="{ 'selected': option === sortRCase }" @click="handleClick(option, index, $event)">
  ...
  <div class="post-wrap">
    <div class="post grid-box" v-for="item in pageReportList" :key="item.REP_NO"
    @click=movetocontent(item.REP_NO)>
      ...
      <div class="btn-area">
        ...
        <button v-else class="btn-small" @click="write_report">작성</button>
      </div>
</template>
...
export default {
  data() {
    return {
      reportList: [], //현재 게시판과 페이지에 맞는 글 리스트
      sortRCase: 0,
      optionsN: '최근 순',
  ...
  methods: {
    ...
    sliceList() {
      ...
      this.pageReportList = this.reportList.slice(start, start + this.onePageCnt);
    },
    async getReportList(sortCCaseNum, sortSCaseNum, sortRCaseNum) {
      let keyword = 'none'
      if (this.keyword != '') {
        keyword = this.keyword;
      }
      try {
        const response = await axios.get(`http://localhost:3000/board/report_list/${sortCCaseNum}/${sortSCaseNum}/${sortRCaseNum}/${keyword}`);
        this.reportList = response.data;
        console.log(this.reportList);
        this.pageCnt = Math.ceil(this.reportList.length / this.onePageCnt)
        this.setPage(1)
      } catch (error) {
        console.error(error);
      }
    ...
    sortList(sortNum, event) {
      ...
      this.getReportList(this.sortCCase, this.sortSCase, this.sortRCase)
    },
    ...
    movetocontent(REP_NO) {
      window.location.href = window.location.pathname + '/reportDetail?REP_NO=' + REP_NO
    },
