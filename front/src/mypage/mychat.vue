        ...
        methods: {
            ...
            gotoChatRoom(index) {
                let popupWindow = window.open(`/chatroom/${index}`, '_blank', 'left=100', 'top=50', 'scrollbars=no', 'resizable=no', 'toolbars=no', 'menubar=no');
                    ...
                    popupWindow.onresize = ( _=> { popupWindow.resizeTo(800, 650) } )
            },
            // 채팅방 리스트 갯수 가져오기
            ...
            async getChatUser(user_no){
                try {
                    ...
                    this.userNameList.push(response.data[0].USER_NM);   // db 대소문자 문제
                } catch (error) {
                    console.error(error);
                }
            },
            ...
            async getAlarm(room_no) {
              try {
                const response = await axios.get(`http://localhost:3000/chat/chat_check_alram/${this.user.user_no}/${room_no}`);
